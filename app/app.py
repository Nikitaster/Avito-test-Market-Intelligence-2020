...
async def stats(search_id: int = Query(..., gt=0),
    ...
    stats_queryset = Stats.filter(search_id=search_id, created_at__gte=from_datetime, created_at__lte=to_datetime)
...
def stats_updater_run():
    """
    Таким образом я реализовал обход ошибки в тестах 'no running event loop'
